...
function ciniki_directory_entryFileGet($ciniki) {
    ...
    $rc = ciniki_directory_checkAccess($ciniki, $args['business_id'], 'ciniki.directory.entryFileGet', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    $strsql = "SELECT ciniki_directory_entry_files.id, "
        ...
        . "WHERE ciniki_directory_entry_files.business_id = '" . ciniki_core_dbQuote($ciniki, $args['business_id']) . "' "
    ...
    if( !isset($rc['file']) ) {
        return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2086', 'msg'=>'Unable to find file'));
    }
