...
function ciniki_directory_entryFileDownload($ciniki) {
    ...
    $rc = ciniki_directory_checkAccess($ciniki, $args['business_id'], 'ciniki.directory.entryFileDownload'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( !isset($rc['file']) ) {
        return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2084', 'msg'=>'Unable to find file'));
    }
    ...
    if( $rc['file']['extension'] == 'pdf' ) {
        ...
        return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2085', 'msg'=>'Unsupported file type'));
    }
    // Specify Filename
