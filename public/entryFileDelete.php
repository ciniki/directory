...
function ciniki_directory_entryFileDelete(&$ciniki) {
    ...
    $rc = ciniki_directory_checkAccess($ciniki, $args['tnid'], 'ciniki.directory.entryFileDelete'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( !isset($rc['file']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.directory.22', 'msg'=>'Unable to find existing item'));
    }
    $uuid = $rc['file']['uuid'];

    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'objectDelete');
    return ciniki_core_objectDelete($ciniki, $args['tnid'], 'ciniki.directory.entry_file', $args['file_id'], $uuid, 0x07);
}
?>
