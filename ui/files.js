...
function ciniki_directory_files() {
	this.init = function() {
		...
		this.add = new M.panel('Add File',
			'ciniki_directory_files', 'add',
			'mc', 'medium', 'sectioned', 'ciniki.directory.info.edit');
		...
		this.add.sections = {
			...
			'_save':{'label':'', 'buttons':{
				'save':{'label':'Save', 'fn':'M.ciniki_directory_files.addFile();'},
			}},
		};
		...
		this.add.addButton('save', 'Save', 'M.ciniki_directory_files.addFile();');
		...
		this.edit.sections = {
			...
			'_save':{'label':'', 'buttons':{
				'save':{'label':'Save', 'fn':'M.ciniki_directory_files.saveFile();'},
				'download':{'label':'Download', 'fn':'M.ciniki_directory_files.downloadFile(M.ciniki_directory_files.edit.file_id);'},
				'delete':{'label':'Delete', 'fn':'M.ciniki_directory_files.deleteFile();'},
			}},
		};
				...
				{'business_id':M.curBusinessID, 'entry_id':M.ciniki_directory_files.add.entry_id}, c,
					function(rsp) {
						if( rsp.stat != 'ok' ) {
							...
							M.ciniki_directory_files.add.file_id = rsp.id;
							M.ciniki_directory_files.add.close();
						}
					});
		} else {
			M.ciniki_directory_files.add.close();
	...
	this.showEditFile = function(cb, fid) {
		...
		var rsp = M.api.getJSONCb('ciniki.directory.entryFileGet', {'business_id':M.curBusinessID, 
			'file_id':this.edit.file_id}, function(rsp) {
				...
				M.ciniki_directory_files.edit.data = rsp.file;
				M.ciniki_directory_files.edit.refresh();
				M.ciniki_directory_files.edit.show(cb);
			});
	};
	...
	this.downloadFile = function(fid) {
		window.open(M.api.getUploadURL('ciniki.directory.fileDownload', {'business_id':M.curBusinessID, 'file_id':fid}));
	};
}
